...
function getBaseURL() {
  ...
  if (!process.env.FX_WEB_BASE_URL && !process.env.CI) {
    ...
    if (environment) {
      ...
      if (process.env.FX_WEB_BASE_URL) {
        return process.env.FX_WEB_BASE_URL;
      }
    }
  }

  /* This is the default URL when running an azure function locally */
  let baseURL = process.env.FX_WEB_BASE_URL || "http://localhost:7071";
